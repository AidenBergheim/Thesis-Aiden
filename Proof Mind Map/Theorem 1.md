...
---
Suppose the **Assumptions** hold. Then under the target estimator and control algorithm, the norm of the target estimation error $\|\tilde{x}_i(t)\|$ converges to zero within strongly predefined time $T_{c,1}$
...
which takes on the exact form of **Lemma 3**. Therefore, according to Lemma 3, the set $M_1 = \{\tilde{x} = 0_2\}$ is GSPTA with the strongly predefined time $T_{c,1}$.