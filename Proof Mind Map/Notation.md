...
$\tilde{d}(t) \in \mathbb{R}$: the error in agent to target centroid estimation $\hat{d}_i(t) - d_i(t)$
...
$\boldsymbol{r}^*(t) \in \mathbb{R}^2$: the desired circumnavigation distance from the shape of circumnavigation.
