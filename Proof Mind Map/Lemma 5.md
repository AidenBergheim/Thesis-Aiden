...
# Proof
---
Assuming $d(t)>0$, using the **Dynamics** we have that $t>T_{c,1}$
...
From **Lemma 4** and **Assumption 1** we have that $d(T_{c,1})=d(0)>d^*(T_{c,1})$, and hence $\delta(T_{c,1})>0$. We also have that both $d(t)$ and $d^*(t)$ are continuous, so it follows that in the case that we get closer to the centroid than the desired distance, so $d(t)\leq d^*(t)$, we have that $\dot{\delta}(t)\geq 0$  and hence we must have that
$$d(t)\geq d^*(t)>0$$
for $t>T_{c,1}$.
