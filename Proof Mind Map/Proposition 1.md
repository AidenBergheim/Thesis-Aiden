...
---
If $P_i(t_1)$ is positive definite at some time $0\leq t_1<T_{c,1}$, then for all $t_1\leq t<T_{c,1}$, $P_i(t)$ remains positive definite.
# Proof
---
We have for $0\leq t_1<T_{c,1}$ the solution
...
So for $t_1\leq t<T_{c,1}$ we have 
...
Term $A$ is positive definite by assumption and term $B$ is at least positive semi-definite for all $\boldsymbol{\varphi}_i(t) \in \mathbb{R}^2$. Since $P_i(t)$ can be expressed as the sum of the positive definite $A$ and positive semi-definite matrix $B$, $P_i(t)$ must be positive definite for all subsequent $t_1\leq t<T_{c,1}$.