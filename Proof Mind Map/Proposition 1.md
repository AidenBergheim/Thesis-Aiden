...
$$\boldsymbol{\dot{\psi}}(t) = \frac{d}{dt}\frac{\boldsymbol{c}(t)-\boldsymbol{y}(t)}{r(t)}$$
Applying produce rule
$$\boldsymbol{\dot{\psi}}(t) = \frac{(\dot{\boldsymbol{c}}(t)-\dot{\boldsymbol{y}}(t))r(t)-(\boldsymbol{c}(t)-\boldsymbol{y}(t))\dot{r}(t)}{r^2(t)}$$

Using **Background 1** again and **Lemma 1** we get

$$\boldsymbol{\dot{\psi}}(t) = \frac{-\dot{\boldsymbol{y}}(t)-\boldsymbol{\psi}(t)\dot{r}(t)}{r(t)}$$

When $t\leq t_s$, using **Dynamics** we have

$$\boldsymbol{\dot{\psi}}(t) = \frac{-\dot{\boldsymbol{y}}(t)}{r(t)}=-\frac{k_{\omega}}{r(t)}\bar{\boldsymbol{\psi}}(t)$$
When $t>t_s$ we get

$$\boldsymbol{\dot{\psi}}(t) = \frac{-u_\psi(t)\boldsymbol{\psi}(t)
- k_{\omega}\,\bar{\boldsymbol{\psi}}(t)+\boldsymbol{\psi}(t)u_\psi(t)}{r(t)}= -\frac{k_{\omega}}{r(t)}\bar{\boldsymbol{\psi}}(t)$$
...
$$\frac{d\beta(t)}{dt}=\frac{k_{\omega}}{r(t)}$$
