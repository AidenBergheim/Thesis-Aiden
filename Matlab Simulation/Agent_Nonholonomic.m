classdef Agent_Nonholonomic
    ...
    methods
        ...
        function obj = controlInputPDTHolonomic(obj, t, dT)
            ...
            obj.u_angle(t) = atan2(obj.u(2), obj.u(1));
        ...
        % Achieves heading convergence but not for discontinuity at t =
        % T_{c,1}
        function obj = controlInputPDTv4(obj, t, dT)
            ...
            theta_d_dot = wrapToPi(theta_d - obj.theta_d_prev) / dT;
            obj.theta_d_prev = theta_d;
            
            % PDT heading controller with feedforward
            pdt_term = -1/(obj.alpha_0 * obj.Tc0) ...
                     * exp(abs(obj.theta_tilde) ^ obj.alpha_0) ...
                     * sig(obj.theta_tilde, 1 - obj.alpha_0);
            
            obj.heading_dot = theta_d_dot + pdt_term;


            % Just making sure agent heading is initially aligned with chi
            % bar and following it
            if t * dT < obj.Tc1
                obj.heading = theta_d;
                obj.heading_dot = 0;
            end
        ...
        % Achieves PDT heading convergence with smooth transitino at t =
        % T{c,1}
        function obj = controlInputPDTv5(obj, t, dT)
            ...
            % Phase 1 command
            u_phase1 = obj.k_omega * obj.localization_heading;
            
            if t_now < obj.Tc1
                obj.u = u_phase1;
            elseif t_now < obj.Tc1 + epsilon
                % Phase 2 command
                v_cen = 1/(obj.alpha_2 * obj.Tc2) ...
                      * exp(abs(obj.d_tilde) ^ obj.alpha_2) ...
                      * sig(obj.d_tilde, 1 - obj.alpha_2) ...
                      - obj.d_des_dot(t);
                u_phase2 = v_cen * obj.psi + obj.k_omega * obj.bar_psi;
                
                % Smooth blending: sigma transitions from 0 to 1 over [Tc1, Tc1 + epsilon]
                s = (t_now - obj.Tc1) / epsilon;
                sigma = 3*s^2 - 2*s^3; % Smooth step (C1, zero derivative at boundaries)
                
                obj.u = u_phase1 + sigma * (u_phase2 - u_phase1);
            else
                v_cen = 1/(obj.alpha_2 * obj.Tc2) ...
                      * exp(abs(obj.d_tilde) ^ obj.alpha_2) ...
                      * sig(obj.d_tilde, 1 - obj.alpha_2) ...
            ...
            % During Phase 1, enforce heading alignment with chi_bar
            if t_now < obj.Tc1
                obj.heading = theta_d;
                obj.heading_dot = 0;
            end
