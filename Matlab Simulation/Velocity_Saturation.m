...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    Agent1 = Agent(p_0, x_hat_0, k_omega, Tc1, Tc2, ...
...
for t = 1:tSteps
    ...
    Agent1 = Agent1.controlInputPDT(t, Tc1, Tc2, dT);                % --- Run control law
    Agent1 = Agent1.move(dT, t);                                % --- Execute control law
...
% 5. DISTANCE ANNOTATION
pt1 = [-1.2, -1.8]; pt2 = [-0.2, -1];
plot(ax1, [pt1(1) pt2(1)], [pt1(2) pt2(2)], '-', 'LineWidth', 1.2, ...
    'Color', color_agent2, 'HandleVisibility', 'off');
plot(ax1, [pt1(1) pt2(1)], [pt1(2) pt2(2)], '.', 'MarkerSize', 12, ...
    'Color', color_agent2, 'HandleVisibility', 'off');
dist_val = norm(pt2 - pt1);
text(ax1, mean([pt1(1) pt2(1)])+0.1, mean([pt1(2) pt2(2)]) - 0.1, ...
    sprintf('%.2f m', dist_val), 'Interpreter', 'latex', 'FontSize', 9, 'Color', color_agent2);
...
ax2_inset = axes('Position', [0.66, 0.44, 0.16, 0.15]);
box(ax2_inset, 'on');
...
rectangle(ax2, 'Position', [x_zoom(1) - 1, y_zoom(1) - 0.02, diff(x_zoom), diff(y_zoom)], ...
