...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    Targets       = [-2, 0, 2, 1; 
                     0, 4, 0, 1];    % initial location of the target 
    ...
    Target_velocities = 2 * [0.05, 0.05, 0.05,  0.05;   % vx for each target
                         0.05, 0.05,   0.05, 0.05];  % vy for each target

    
    % ---- Pre-allocate Target Trajectory Storage ----
    Target_traj = zeros(2, qtyTargets, tSteps);
    Target_traj(:, :, 1) = Targets;  % Store initial positions
    ...
    % ---- Initial Conditions ----
        ...
        p_0             = [7; 0];      % initial location of the agent 
    ...
    Agent = Agent(p_0, x_hat_0, k_omega, Tc1, Tc2, ...
                        alpha_1, alpha_2, d_des_handle, tSteps, qtyTargets, Targets, initial_heading, 5, 0);
...
for t = 1:tSteps
    ...
    Agent = Agent.estimateTargetPDTIndefinite(t, dT, Targets, Tc1);          % --- Run estimator
    Agent = Agent.controlInputPDT(t, Tc1, Tc2, dT);                % --- Run control law
    Agent = Agent.move(dT, t);                                % --- Execute control law

    time = (t-1)*dT;
    velocities = targetVelocityFunction(time);
    
    [Targets, Target_traj] = moveTargets(Targets, velocities, dT, Target_traj, t);
end
...
%  ---- (a) Agent Trajectory plot -----
    ...
    for i = 1:qtyTargets
        ...
        traj_i = squeeze(Target_traj(:, i, 1:mytStepFinal));  % 2 x mytStepFinal
        plot(ax1, traj_i(1,:), traj_i(2,:), ...
            'LineWidth', 1.2, ...
            'Color', targetColors(i,:), ...
            'DisplayName', ['$$\mbox{\boldmath$x$}_' num2str(i) '(t)$$']);
    ...
    for i = 1:qtyTargets
        ...
        traj_i = squeeze(Target_traj(:, i, 1:mytStepFinal));  % 2 x mytStepFinal
        plot(ax1, traj_i(1,1), traj_i(2,1), 'ks', ...
            'LineWidth', 1, 'MarkerSize', 7, ...
            'Color', targetColors(i,:), ...
            'DisplayName', ['$$\mbox{\boldmath$x$}_' num2str(i) '(0)$$']);
    ...
    % Plot final agent position
     plot(Agent.p_traj(1,mytStepFinal), Agent.p_traj(2,mytStepFinal), 'o', ...
        ...
        'LineWidth', 1.5, ...
        'DisplayName', '$$\mbox{\boldmath$y$}(140)$$');
...
% ------- (b) Estimate Trajectories plot --------
    ...
    for i = 1:qtyTargets
        ...
        plot(ax2, xT, yT, '+', ...
            ...
            'LineWidth', 1, ...
            'MarkerSize', 7, ...
            'DisplayName', ['$$\mbox{\boldmath$x$}_' num2str(i)  '(' num2str(tFinal) ')$$']);
    end

    % Plot TRUE target trajectories
    for i = 1:qtyTargets
        traj_i = squeeze(Target_traj(:, i, 1:mytStepFinal));  % 2 x T
        
        plot(ax2, traj_i(1,:), traj_i(2,:), ...
            'LineWidth', 1.2, ...
            'Color', 0.8 * targetColors(i,:), ...
            'DisplayName', ['$$\mbox{\boldmath$x$}_' num2str(i) '(t)$$']);
    end
...
x_zoom = [20, 60];      % x range to zoom into - adjust to your data
y_zoom = [-0.05, 0.05];  % y range to zoom into - adjust to your data

ax3_inset = axes('Position', [0.74, 0.65, 0.16, 0.08]); % [left bottom width height] in figure coords
box(ax3_inset, 'on');
...
% Bottom-left corner of rectangle -> top-left of inset
p1 = data2fig(ax3, x_zoom(1), y_zoom(1)); % bottom-left of rectangle
p2 = [ax3_inset.Position(1), ...                          % left
      ax3_inset.Position(2) + ax3_inset.Position(4)];     % top
annotation('line', [p1(1), p2(1)], [p1(2), p2(2)], ...
    'Color', [0, 0.6, 0], 'LineStyle', '--', 'LineWidth', 1);

% Bottom-right corner of rectangle -> top-right of inset
p3 = data2fig(ax3, x_zoom(2), y_zoom(1)); % bottom-right of rectangle
p4 = [ax3_inset.Position(1) + ax3_inset.Position(3), ...  % right
      ax3_inset.Position(2) + ax3_inset.Position(4)];     % top
...
x_zoom = [45, 90];      % x range to zoom into - adjust to your data
y_zoom = [-0.1, 0.4];  % y range to zoom into - adjust to your data

ax4_inset = axes('Position', [0.72, 0.24, 0.1, 0.1]); % [left bottom width height] in figure coords
box(ax4_inset, 'on');
hold(ax4_inset, 'on')
    for i = 1:qtyTargets
        err_norm_i = vecnorm(Agent.x_tilde_traj{i}(:, 1:mytStepFinal));
...
rectangle(ax4, 'Position', [x_zoom(1), y_zoom(1) - 0.1, diff(x_zoom), 1.5*diff(y_zoom)], ...
    'EdgeColor', [0, 0.6, 0], 'LineWidth', 1.2);
...
p1 = data2fig(ax4, x_zoom(1), y_zoom(2));
p2 = [ax4_inset.Position(1), ax4_inset.Position(2)];
annotation('line', [p1(1), p2(1)], [p1(2), p2(2)], ...
    'Color', [0, 0.6, 0], 'LineStyle', '--', 'LineWidth', 1);

% Top-right corner of rectangle -> bottom-right of inset
p3 = data2fig(ax4, x_zoom(2), y_zoom(2));
p4 = [ax4_inset.Position(1) + ax4_inset.Position(3), ax4_inset.Position(2)];
annotation('line', [p3(1), p4(1)], [p3(2), p4(2)], ...
    'Color', [0, 0.6, 0], 'LineStyle', '--', 'LineWidth', 1);


function velocities = targetVelocityFunction(t)

    % Example: sinusoidal + exponential decay
    vy = 0.2*(0.1 + 0.1*exp(-0.05*t) * abs(sin(0.2*t)));
    vx = 0.2*(-0.5 * exp(-0.05*t)*abs(cos(0.2*t)));

    % Apply same pattern to all targets (or customise per target)
    velocities = [vx, vx, vx, vx;
                  vy, vy, vy, vy];
end
...
function [Targets, Target_traj] = moveTargets(Targets, velocities, dT, Target_traj, t)
% MOVETARGETS  Advances each target by one constant-velocity step and
%              logs the new position into the trajectory store.
%
%   Inputs:
%     Targets      - 2 x N matrix of current target positions
%     velocities   - 2 x N matrix of constant velocities (m/s)
%     dT           - time step (s)
%     Target_traj  - 2 x N x tSteps pre-allocated trajectory array
%     t            - current time step index
%
%   Outputs:
%     Targets      - 2 x N matrix of updated target positions
%     Target_traj  - updated trajectory array with new positions at index t+1

    Targets = Targets + velocities * dT;

    % Guard against writing past the end of the pre-allocated array
    if t + 1 <= size(Target_traj, 3)
        Target_traj(:, :, t + 1) = Targets;
    end
end
