...
for t = 1:tSteps
    ...
    AgentPDT = AgentPDT.estimateTarget(t, dT, Targets);         % --- Run estimator
    AgentPDT = AgentPDT.getPsiAndProjection(t, qtyTargets);     % --- Estimate Convex Hull
    AgentPDT = AgentPDT.getVelocity(t);                         % --- Run control law
    ...
    %AgentCao = AgentCao.estimateTarget(t, dT, Targets);         % --- Run estimator
    %AgentCao = AgentCao.getPsiAndProjection(t, qtyTargets);     % --- Estimate Convex Hull
    %AgentCao = AgentCao.getVelocity(t);                         % --- Run control law
    %AgentCao = AgentCao.move(dT, t);                            % --- Execute control law
    
    NonHolonomic = NonHolonomic.getBearings(t, Targets);                % --- Take bearing measurements
    NonHolonomic = NonHolonomic.estimateTarget(t, dT, Targets);         % --- Run estimator
    NonHolonomic = NonHolonomic.getPsiAndProjection(t, qtyTargets);     % --- Estimate Convex Hull
    NonHolonomic = NonHolonomic.getVelocity(t);                         % --- Run control law
...
for i = 1:qtyTargets
    ...
    % Extract target position (static)
    xT = Targets(1, i);
    yT = Targets(2, i);

    % Plot target with distinct color
    plot(xT, yT, 'r+', ...
        'LineWidth', 0.7, ...
        'MarkerSize', 7, ...
        'DisplayName', ['$\mathbf{x}_' num2str(i) '$']);
end
...
plot(AgentPDT.delta_traj,...
    'DisplayName', '$$\delta(t)=d(t)-d^*$$ [Sui et al. (2025)]', ...
    'LineWidth', 1.6, ...
    'Color'      ,  'b');
hold on

%plot(AgentCao.delta_traj,...
%    'DisplayName', '$$\delta(t)=d(t)-d^*$$ [Cao et al. (2021)]', ...
%    'LineWidth', 1.6, ...
%    'Color'      ,  'r');
%hold on
