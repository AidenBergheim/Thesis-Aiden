...
h_start = plot(Agent.p_traj(1,1), Agent.p_traj(2,1), 'o', ...
    'MarkerSize', 8, ...
    'MarkerEdgeColor', 'k', ...
...
h_path_PDT = plot(Agent.p_traj(1,mytStepFinal), Agent.p_traj(2,mytStepFinal), 'o', ...
    'MarkerSize', 8, ...
    'MarkerEdgeColor', 'k', ...
    'MarkerFaceColor', 'k', ...
    'LineWidth', 1.5, ...
...
h_final_PDT = plot(ax1, Agent.p_traj(1,:), Agent.p_traj(2,:), ...
    'LineWidth', 1, ...
    'Color', 'k', ...
...
% --- Distance annotation between two points ---
pt1 = [-1.2, -1.8]; pt2 = [-0.2, -1];
plot(ax1, [pt1(1) pt2(1)], [pt1(2) pt2(2)], 'r-', 'LineWidth', 1.2, 'HandleVisibility', 'off');
plot(ax1, [pt1(1) pt2(1)], [pt1(2) pt2(2)], 'r.', 'MarkerSize', 12, 'HandleVisibility', 'off');
dist_val = norm(pt2 - pt1);
text(ax1, mean([pt1(1) pt2(1)])+0.1, mean([pt1(2) pt2(2)]) - 0.1, ...
    sprintf('%.2f m', dist_val), 'Interpreter', 'latex', 'FontSize', 9, 'Color', 'r');
...
%  ---- (c) Tracking Error Plot -----
    ...
    plot(ax2, t_vec, real(Agent.delta_traj(1:mytStepFinal)),...
        'DisplayName', '$$\delta(t)$$', ...
        'LineWidth', 1, ...
        'Color'      ,  'k');
    hold on
...
x_zoom = [6, 7];      % x range to zoom into - adjust to your data
y_zoom = [-0.05, 0.4];  % y range to zoom into - adjust to your data

ax2_inset = axes('Position', [0.66, 0.44, 0.16, 0.2]); % [left bottom width height] in figure coords
box(ax2_inset, 'on');
plot(ax2_inset, t_vec, real(Agent.delta_traj(1:mytStepFinal)), ...
    'LineWidth', 1, 'Color', 'k');
...
rectangle(ax2, 'Position', [x_zoom(1) - 1, y_zoom(1) - 0.02, 3*diff(x_zoom), diff(y_zoom)], ...
...
p1 = data2fig(ax2, x_zoom(2), y_zoom(2)); % top-right of rectangle
p2 = [ax2_inset.Position(1), ...                          % left
      ax2_inset.Position(2) + ax2_inset.Position(4)];     % top
...
p3 = data2fig(ax2, x_zoom(2), y_zoom(1)); % bottom-right of rectangle
p4 = [ax2_inset.Position(1), ...          % left
      ax2_inset.Position(2)];             % bottom
