...
figure('Position', [50 100 1040 420]); % Create a new figure
...
x_zoom = [20, 100];      % x range to zoom into - adjust to your data
y_zoom = [-0.3, 0.3];  % y range to zoom into - adjust to your data

ax3_inset = axes('Position', [0.74, 0.72, 0.16, 0.12]); % [left bottom width height] in figure coords
box(ax3_inset, 'on');
...
rectangle(ax3, 'Position', [x_zoom(1), y_zoom(1), diff(x_zoom),  diff(y_zoom)], ...
...
ax4_inset = axes('Position', [0.72, 0.24, 0.1, 0.14]); % [left bottom width height] in figure coords
box(ax4_inset, 'on');
hold(ax4_inset, 'on')
    for i = 1:qtyTargets
        err_norm_i = vecnorm(Agent.x_tilde_traj{i}(:, 1:mytStepFinal));
