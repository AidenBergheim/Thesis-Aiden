...
% ------ (d) Estimation Error Plot ---------
   ...
   % --- Add Vertical Line for Estimation Convergence (Tc1) ---
    ...
    hold on;
    plot(T.t(start_index:end_index) - start_time, sqrt(((2* T.u_x_t_(startIndex:end_index))).^2 + (2*(T.u_y_t_(startIndex:end_index))).^2),...
...
ax5 = nexttile(6, [1 1]); 
    ...
    plot(T.t(start_index:end_index-1) - start_time, movmean(sqrt((vx - 2*T.u_x_t_(startIndex:end_index-1)).^2 + (vy - 2*T.u_y_t_(startIndex:end_index-1)).^2), 10),...
        'DisplayName', '$$|v(t) - u(t)|$$: Actuation Error', ...
        'LineWidth', 1)
