...
u_commanded = [2*T.u_x_t_, 2*T.u_y_t_];     % Mx2
...
u_centripetal = ... 
    2.* sum(u_theory .* psi_theory,2) .* psi_theory;
...
% ------ (d) Estimation Error Plot ---------
    ...
    pT_hat2 = [T.pT_hat1_x_t_(start_index:end)'; T.pT_hat1_y_t_(start_index:end)'];
    err_norm_2 = vecnorm(Targets(:,1) - pT_hat2, 2, 1);
    hold on
    % Plotting norm against t_vec
    ...
    pT_hat3 = [T.pT_hat1_x_t_(start_index:end)'; T.pT_hat1_y_t_(start_index:end)'];
    err_norm_3 = vecnorm(Targets(:,1) - pT_hat3, 2, 1);
    hold on
    % Plotting norm against t_vec
