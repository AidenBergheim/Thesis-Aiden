...
%  ---- (a) Agent Trajectory plot -----
    ...
    h_targets_final(1) = plot(ax1, Targets(1,1), Targets(2,1), 'k+', ...
        'LineWidth', 1, 'MarkerSize', 7, 'Color', targetColors(1, :),...
        'DisplayName', '$$\mbox{\boldmath$x$}_1$$');

    h_targets_final(2) = plot(ax1, Targets(1,2), Targets(2,2), 'k+', ...
        'LineWidth', 1, 'MarkerSize', 7, 'Color', targetColors(2, :),...
        'DisplayName', '$$\mbox{\boldmath$x$}_2$$');

    h_targets_final(3) = plot(ax1, Targets(1,3), Targets(2,3), 'k+', ...
        'LineWidth', 1, 'MarkerSize', 7, 'Color', targetColors(3, :),...
    ...
    h_final = plot(T.p_a_x_t_(end_index), T.p_a_y_t_(end_index), 'o', ...
        'MarkerSize', 8, 'MarkerEdgeColor', 'k', 'MarkerFaceColor', [0, 0, 0], ...
        'LineWidth', 1.5, 'DisplayName', '$$\mbox{\boldmath$y$}(t_f)$$');

    % Start Position
    h_start = plot(T.p_a_x_t_(takeoff_index), T.p_a_y_t_(takeoff_index), 'o', ...
        'MarkerSize', 8, 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'none', ...
        'LineWidth', 1.5, 'DisplayName', '$$\mbox{\boldmath$y$}(0)$$');
...
%  ---- (b) Tracking Error Plot -----
    ...
    plot(ax2, T.t(start_index:end_index) - start_time, err_norm_1, ...
        'LineWidth', 1.3, 'LineStyle', '--', 'Color', targetColors(1,:),...
    ...
    plot(ax2, T.t(start_index:end_index) - start_time, err_norm_2, ...
        'LineWidth', 1.3, 'LineStyle', '--', 'Color', targetColors(2,:),...
    ...
    plot(ax2, T.t(start_index:end_index) - start_time, err_norm_3, ...
        'LineWidth', 1.3, 'LineStyle', '--', 'Color', targetColors(3,:),...
        'DisplayName', '$$\|\mbox{\boldmath$\tilde{x}$}_3(t)\|$$');

    if exist('Tc1', 'var')
        xline(ax2, Tc1, '-.', {'$T_{c,1}$'}, ...
            'Color', 'r', ...
            'Interpreter', 'latex', ...
            'FontSize', 8, ...
            'LabelVerticalAlignment', 'top', ...
    ...
    ylim(ax2, [-0.2, max(1.2 * max(d_tilde(:)), 0.8)])
...
ax2_inset = axes('Position', [0.68, 0.72, 0.12, 0.12]); % [left bottom width height] in figure coords
box(ax2_inset, 'on');
