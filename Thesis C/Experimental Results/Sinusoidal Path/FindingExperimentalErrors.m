...
u_commanded = [T.u_x_t_, T.u_y_t_];     % Mx2
u_commanded = u_commanded(startIndex:end_index,:);

t_plot = T.t(startIndex:end_index) - start_time;
...
plot(ax1, t_plot, u_centripetal_signed, ...
    'LineWidth',2,'DisplayName','Theoretical Centripetal u');

plot(ax1, t_plot, u_cmd_centripetal_signed, ...
    'LineWidth',2,'DisplayName','Commanded Centripetal u');
...
plot(ax2, t_plot, u_tangential_signed, ...
    'LineWidth',2,'DisplayName','Theoretical Tangential u');

plot(ax2, t_plot, u_cmd_tangential_signed, ...
    'LineWidth',2,'DisplayName','Commanded Tangential u');
...
%  ---- (a) Agent Trajectory plot -----
    ...
    plot(ax1, T.pT_hat1_x_t_(1:end_index), T.pT_hat1_y_t_(1:end_index), ...
        'LineWidth', 1, ...
        'Color', targetColors(1, :),...
        'LineStyle'  ,  '--', ...
        'DisplayName', '$$\mbox{\boldmath$\hat{x}$}_1(t)$$');

    plot(ax1, T.pT_hat2_x_t_(1:end_index), T.pT_hat2_y_t_(1:end_index), ...
        'LineWidth', 1, ...
        'Color', targetColors(2, :),...
        'LineStyle'  ,  '--', ...
        'DisplayName', '$$\mbox{\boldmath$\hat{x}$}_1(t)$$');

    plot(ax1, T.pT_hat3_x_t_(1:end_index), T.pT_hat3_y_t_(1:end_index), ...
        'LineWidth', 1, ...
    ...
    % Plotting PDT Agent
    plot(ax1, T.p_a_x_t_(startIndex:end_index), T.p_a_y_t_(startIndex:end_index), ...
        'LineWidth', 1, ...
...
%  ---- (c) Tracking Error Plot -----
    ...
    % --- Axis Properties ---
    xlim(ax2, [0, T.t(end_index)])
    ylim(ax2, [1.1 * min(d_tilde(:)), max(max(d_tilde(:)), 0.8)])
...
% ------ (d) Estimation Error Plot ---------
   ...
   % --- Add Vertical Line for Estimation Convergence (Tc1) ---
    ...
    plot(T.t(start_index+1:end_index) - start_time, movmean(sqrt(vx.^2 + vy.^2), 10), 'DisplayName', '$$v(t)$$: Velocity', 'LineWidth', 1)
    %hold on;
    %plot(T.t(start_index:end_index) - start_time, sqrt(T.v_a_x_t_(startIndex:end_index).^2 + T.v_a_y_t_(startIndex:end_index).^2),...
    %        'DisplayName', 'Actual $$v(t)$$', ...
    %        'LineWidth', 1)

    hold on;
    plot(T.t(start_index:end_index) - start_time, sqrt((T.u_x_t_(startIndex:end_index)).^2 + (T.u_y_t_(startIndex:end_index)).^2),...
        'DisplayName', '$$u(t)$$: Control Effort', ...
        'LineWidth', 1)

    %hold on;
    %plot(T.t(start_index:end_index-1) - start_time, movmean(sqrt(vx.^2 + vy.^2), 10) - sqrt((T.u_x_t_(startIndex:end_index-1)).^2 + (T.u_y_t_(startIndex:end_index-1)).^2),...
    %    'DisplayName', '$$v(t) - u(t)$$: Actuation Error', ...
    %    'LineWidth', 1)

    xlim(ax4, [0, T.t(end_index)])
...
ax5 = nexttile(6, [1 1]); 
    ...
    plot(T.t(start_index:end_index-1) - start_time, movmean(sqrt((vx - T.u_x_t_(startIndex:end_index-1)).^2 + (vy - T.u_y_t_(startIndex:end_index-1)).^2), 10),...
        'DisplayName', '$$|v(t) - u(t)|$$: Actuation Error', ...
        'LineWidth', 1)

    plot(ax5, T.t(start_index:end_index) - start_time, d_tilde(:),...
        'DisplayName', '$$|\delta(t)|$$', ...
        'LineWidth', 1, ...
        'Color'      ,  'k');

    xlim(ax5, [0, T.t(end_index)])
    title(ax5, '(e) Actuation Errors', 'Interpreter','latex')
    xlabel(ax5, 'time (sec)', 'Interpreter','latex')
    ylabel(ax5, 'Velocity (m/s) or Errors (m)', 'Interpreter','latex')
    grid(ax5, 'on');
    set(ax5, 'FontSize', 10);
...
t = tiledlayout(3,1);
%plot(T.t(start_index+1:end_index) - start_time, movmean(sqrt(vx.^2 + vy.^2), 10), 'DisplayName', '$$\dot{y}(t)$$', 'LineWidth', 1)
    %hold on;
    %plot(T.t(start_index:end_index) - start_time, sqrt(T.v_a_x_t_(startIndex:end_index).^2 + T.v_a_y_t_(startIndex:end_index).^2),...
    %        'DisplayName', 'Actual $$v(t)$$', ...
    %        'LineWidth', 1)

   % plot(T.t(start_index:end_index) - start_time, movmean(sqrt((2*T.u_x_t_(startIndex:end_index)).^2 + (2 * T.u_y_t_(startIndex:end_index)).^2), 10),...
        %'DisplayName', '$$u(t)$$', ...
        %'LineWidth', 1)
