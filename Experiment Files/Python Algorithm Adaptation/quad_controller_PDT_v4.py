...
for the corresponding paper. The code is inspired by
the python adaptation of proposed algorithms written by Donglin Sui: https://github.com/Gloogger
...
def minEnclosingEllipseStat(points):
    ...
    # --- Calculate Covariance Matrix and Eigenvectors and Eigenvalues ---
    ...
    # --- Calculate Semi-Axis Lengths and sort ---
...
def my_PDT_BoTLC_control():
    ...
    target_index = np.array([1, 2, 3])        # The second agent in Agents is the stationary target
    ...
    pT_hat_x0 = np.array([1.0, 0.5, 0.3])
    pT_hat_y0 = 1/4 * np.array([0, 0.234, 0])
    ...
    k_omega = 0.3    # Control gain, k_omega > 0
    ...
    p_T_x_0 = np.array([-0.25, 0.2, 0.25])
    p_T_y_0 = np.array([0, 0.2, 0])
...
# //////////////////////////////////////////////////////////////////////////////
# //                    Calculate Chi and Chi_bar vectors                     //
# //////////////////////////////////////////////////////////////////////////////

    # Get initial agent position
    p_a_x_init, p_a_y_init = qu.get_2D_pos(agent_index)

    # Initial target positions
    targets_init = p_T[:, 0, :]  # [2 x num_targets]

    # Calculate theta_i for each target relative to agent's position
    theta_i = np.zeros(num_targets)
    for i in range(num_targets):
        vec_to_target = targets_init[:, i] - np.array([p_a_x_init, p_a_y_init])
        theta_i[i] = np.arctan2(vec_to_target[1], vec_to_target[0])
        # Normalize to [-pi, pi]
        theta_i[i] = np.arctan2(np.sin(theta_i[i]), np.cos(theta_i[i]))

    # Generate all possible binary vectors k in {0,1}^n
    num_combinations = 2**num_targets
    min_width = np.inf
    k_star = np.zeros(num_targets)

    for idx in range(num_combinations):
        # Convert index to binary vector k
        k = np.zeros(num_targets)
        temp_idx = idx
        for j in range(num_targets-1, -1, -1):
            k[j] = temp_idx % 2
            temp_idx = temp_idx // 2
        
        # Calculate vartheta = theta_i + 2*pi*k_i
        vartheta = theta_i + 2*np.pi*k
        
        # Calculate w(k) = sup(vartheta) - inf(vartheta)
        w_k = np.max(vartheta) - np.min(vartheta)
        
        # Check if this is the minimum width
        if w_k < min_width:
            min_width = w_k
            k_star = k.copy()

    # Calculate optimal vartheta with k_star
    vartheta_star = theta_i + 2*np.pi*k_star

    # Calculate theta_max and theta_min
    theta_max = np.max(vartheta_star)
    theta_min = np.min(vartheta_star)

    # Calculate Chi as the direction of the angular cone bisector
    theta_mid = (theta_max + theta_min) / 2
    chi_angle = theta_mid

    # Chi is the unit vector in the direction of theta_mid
    Chi = np.array([np.cos(chi_angle), np.sin(chi_angle)])

    # Chi_bar is Chi rotated by pi/2 (clockwise)
    Chi_bar = np.array([-Chi[1], Chi[0]])
    ...
    param_header = ['k_omega', 'alpha_1', 'T_c1', 'alpha_2', 'T_c2']
    param_values = [k_omega, alpha_1, T_c1, alpha_2, T_c2]
    ...
    data_header = [
        ...
        'pC_hat_x(t)', 'pC_hat_y(t)',
        'u_x(t)', 'u_y(t)',                                  # control input
        'z',                                                 # current altitude
        'd_des(t)'                                           # desired distance to centroid
    ]
    ...
    while time.time() - start_time < qgv.RUN_TIME:
        ...
        psi_x = ( - p_a_x + pC_x ) * 1/d
        psi_y = ( - p_a_y + pC_y ) * 1/d
        ...
        elapsed_controller_time = time.time() - control_start_time
        
        for i in range(num_targets):
            # Update P_dot (matrix)
            if (elapsed_controller_time) <= T_c1:
        ...
        # Finding psi hat
        if d_hat > 1e-6:
            psi_hat_x = ( - p_a_x + pC_hat_x ) * 1/d_hat
            psi_hat_y = ( - p_a_y + pC_hat_y ) * 1/d_hat
        else:
            psi_hat_x = 0
            psi_hat_y = 0

        psi_hat_vec = np.array([psi_hat_x, psi_hat_y])

        bar_psi_hat_x = -psi_hat_y
        bar_psi_hat_y = psi_hat_x

        bar_psi_hat_vec = np.array([bar_psi_hat_x, bar_psi_hat_y])
        ...
        # Define the Scaled Ellipse Matrix 'A'
        #a = semi_axes[0] + r_s
        #b = semi_axes[1] + r_s
        ...
        if tStep >= 100:
            if Up_in_the_Sky: 
                ...
                if (elapsed_controller_time) <= T_c1:
                    # Tangential term: k_omega * Chi_bar
                    tangential_term = k_omega * Chi_bar
                ...
                # After time constant 1 T_c1
                    ...
                    centripetal_term = ((np.exp(np.abs(d_tilde) ** alpha_2) / (alpha_2 * T_c2)) * sig_term + f_int[tStep] - d_des_dot) * psi_hat_vec

                    # Tangential term: k_omega * bar_varphi
                    tangential_term = k_omega * bar_psi_hat_vec
        ...
        t = time.time()
        time.sleep(time_interval)
        elapsed_controller_time = t - control_start_time
