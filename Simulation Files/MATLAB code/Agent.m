...
classdef Agent
    ...
    methods
        ...
        function obj = updateDesiredDistance(obj, cur_tStep, dT)
            ...
            % Finding zeta
            zeta = atan2(obj.p(2) - obj.c_hat(2), obj.p(1) - obj.c_hat(1));
            obj.theta_traj(t) = zeta;
            obj.d_des = obj.d_des_func(current_time, zeta, x_hat_positions, obj.c_hat, obj.p);
            
            % Recording desired distance
            obj.d_des_traj(t) = obj.d_des;

            % Calculating derivative using backwards differentiation - NB
            % this is for the general case, ideally swapped out for each
            % shape
            if t > 1
        ...
        % Getting bearing measurement to each target with Gaussian noise
        ...
        % Individual target estimator inspired by Sui et al. (2025)
